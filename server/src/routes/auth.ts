...
router.post('/register', async (req: Request<object, object, RegisterDTO>, res: Response) => {
  ...
  const newUser = db.createUser({ username, email, password: hashedPassword, avatar });

  // Auto-create conversations with first 3 existing users
  const others = db.getUsers().filter((u) => u.id !== newUser.id).slice(0, 3);
...
router.get('/users', (_req: Request, res: Response) => {
  const users = db.getUsers().map(({ id, username, email, avatar, isOnline }) => ({
    id,
    username,
    email,
    avatar,
