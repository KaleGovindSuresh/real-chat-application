...
function ConversationDropTarget({
  conv,
  isActive,
  onClick,
  currentUserId,
  ...
  const otherParticipants =
    conv.participantUsers?.filter((p) => p.id !== currentUserId) || [];
  const displayName =
    otherParticipants.length === 1 ? otherParticipants[0].username : conv.name;
  const avatar =
    otherParticipants.length === 1 ? otherParticipants[0].avatar : undefined;
  const isOnline =
    otherParticipants.length === 1 ? otherParticipants[0].isOnline : undefined;
