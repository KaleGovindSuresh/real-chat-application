...
export default function MessageInput({ roomId }: Props) {
  ...
  const handleSend = useCallback(
    ...
    [text, result, sendMessage, roomId, clear, removeDraft, stopTypingImmediate],
  );
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              {previewUrl && (
                <div style={{ position: 'relative' }}>
                  {previewType === 'image' ? (
                    <img src={previewUrl} alt="Preview" style={{ height: 80, borderRadius: 'var(--radius-md)', objectFit: 'cover' }} />
                  ) : (
                    <video src={previewUrl} style={{ height: 80, borderRadius: 'var(--radius-md)' }} />
                  )}
                  <motion.button
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ height: 4, borderRadius: 2, background: 'var(--bg-hover)', overflow: 'hidden' }}>
                    <motion.div
                      ...
                      style={{ height: '100%', background: 'linear-gradient(90deg, var(--color-primary), var(--color-accent))', borderRadius: 2 }}
                    />
                  </div>
                </div>
              )}
