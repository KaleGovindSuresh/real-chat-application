...
export default function DragOverlay({ active }: Props) {
  ...
  const data = active.data.current as { type: string; message: Message } | undefined;
  ...
  return (
    <DNDDragOverlay dropAnimation={null}>
      <div className="drag-overlay" style={{
        ...
        boxShadow: '0 12px 40px rgba(0,0,0,0.5), 0 0 0 1px rgba(99,102,241,0.3)',
        ...
        <div style={{
          ...
          {message.type === 'image' && <FiImage size={14} color="var(--text-secondary)" />}
          {message.type === 'video' && <FiVideo size={14} color="var(--text-secondary)" />}
          <span style={{
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          }}>
            {message.type === 'text'
              ? message.content
              : message.content || `${message.type === 'image' ? 'Image' : 'Video'} message`}
          </span>
        </div>
